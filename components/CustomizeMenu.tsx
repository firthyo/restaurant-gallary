...
const CustomizeMenu = forwardRef<CustomizeMenuHandle, CustomizeMenuProps>(
  ({ initialSelections = [] }, ref) => {
    const [selections, setSelections] = useState<string[]>(initialSelections);
    const [circles, setCircles] = useState<{
      [key: string]: { path: string; length: number };
    }>({});
    ...
    const generateFreehandCircle = (width: number, height: number) => {
      ...
      for (let i = 0; i <= points; i++) {
        ...
        const x =
          centerX + Math.cos(angle) * radiusX * (0.9 + Math.random() * 0.2);
        const y =
          centerY + Math.sin(angle) * radiusY * (0.9 + Math.random() * 0.2);
        path += `${i === 0 ? "M" : "L"} ${x},${y} `;
      }
      path += "Z";
    ...
    return (
      <div className="bg-[#FFF5E6] p-6 rounded-lg">
        <h2 className="text-3xl font-bold text-[var(--olive-green)] mb-6 font-sans">
          Customize Your Menu
        </h2>
          ...
          <div key={category} className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 justify-items-center">
              {items.map((item) => (
                ...
                >
                  <span className="font-cursive text-xl relative z-10">
                    {item}
                  </span>
                  {circles[item] && (
                    <svg
            ...
            {index < Object.entries(menuItems).length - 1 && (
              <div
                className="my-6 border-b border-[var(--olive-green)] opacity-30"
                style={{
                  backgroundImage:
                    "url(\"data:image/svg+xml,%3Csvg width='100%25' height='100%25' xmlns='http://www.w3.org/2000/svg'%3E%3Cline x1='0' y1='0' x2='100%25' y2='0' stroke='%235B8C5A' stroke-width='4' stroke-dasharray='1, 10' /%3E%3C/svg%3E\")",
                  height: "4px",
                }}
              />
            )}
        ...
        <div className="mt-6 text-center">
          <p className="text-lg mb-2 font-cursive">
            You have selected {selections.length} item
