...
export default function Booking() {
  ...
  const canProceed = () => {
    switch (step) {
      case 1: return bookingData.table !== null
      case 2: return bookingData.dateTime !== null
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow container mx-auto px-4 py-12 max-w-4xl">
        <h1 className="text-4xl md:text-5xl font-serif italic text-primary text-center mb-6">Make a Reservation</h1>
        
        {/* Stepper */}
        <div className="flex justify-between items-center mb-8">
          {['Select Table', 'Choose Date & Time', 'Customer Info', 'Confirm Booking'].map((stepName, index) => (
            <div key={index} className="flex flex-col items-center">
              <div className={cn(
                "w-8 h-8 rounded-full flex items-center justify-center text-primary-foreground mb-2",
                step > index + 1 ? "bg-primary" : step === index + 1 ? "bg-accent" : "bg-muted"
              )}>
                {index + 1}
              </div>
        ...
        {step === 1 && (
          <div>
            <h2 className="text-2xl font-bold text-accent mb-4">Step 1: Select a Table</h2>
              ...
              {tables.map((table) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "h-48 rounded-lg relative transition-all duration-200",
                      status === 'selected' && "bg-primary text-primary-foreground",
                      status === 'unavailable' && "bg-muted cursor-not-allowed",
                      status === 'available' && "bg-card border-2 border-primary hover:border-accent"
                    )}
                  >
                    <Badge
                      className={cn(
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <h3 className="text-2xl font-bold mb-2">Table {table.id}</h3>
        ...
        {step === 2 && (
          <div>
            <h2 className="text-2xl font-bold text-accent mb-4">Step 2: Choose Date & Time</h2>
        ...
        {step === 3 && (
          <div>
            <h2 className="text-2xl font-bold text-accent mb-4">Step 3: Customer Information</h2>
        ...
        {step === 4 && (
          <div>
            <h2 className="text-2xl font-bold text-accent mb-4">Step 4: Confirm Booking</h2>
            <div className="bg-card p-6 rounded-lg shadow-md mb-6">
              <h3 className="text-xl font-semibold mb-4">Booking Summary</h3>
              <p><strong>Table:</strong> {tables.find(t => t.id === bookingData.table)?.name}</p>
              <p><strong>Date & Time:</strong> {bookingData.dateTime?.toLocaleString()}</p>
